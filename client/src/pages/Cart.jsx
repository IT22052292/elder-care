...
export default function Cart() {
  ...
  const removeFromCart = (index, price) => {
    ...
    sessionStorage.setItem("cart", JSON.stringify(updatedCart)); //updating session storage

    //the total reduction when item removed
    const newTotal = total - removedItem.servicePrice;
    sessionStorage.setItem("total", JSON.stringify(newTotal)); //new total
  ...
  const confirmOrder = async () => {
    try {
      ...
      const res = await axios.post('/API/order/createOrder', orderData);
      const data = res.data;
      ...
      setMessage("You cannot place new orders while having unfulfilled orders.");
  ...
  return (
    <section class="bg-white py-8 antialiased dark:bg-gray-900 md:py-16">
      <div class="mx-auto max-w-screen-xl px-4 2xl:px-0">
        ...
        {cart.length === 0 ? (
          <p class="text-gray-500 dark:text-gray-400 text-center mt-4">There are no added services in the cart. Add some.</p>
        ) : (
                  ...
                  <div class="rounded-lg border border-gray-200 bg-white p-4 shadow-sm dark:border-gray-700 dark:bg-gray-800 md:p-6" key={index}>
                    <div class="space-y-4 md:flex md:items-center md:justify-between md:gap-6 md:space-y-0">
                      <a href="#" class="shrink-0 md:order-1">
                        <img class="h-20 w-20 dark:hidden ml-2" src="https://www.pngall.com/wp-content/uploads/8/Service-PNG-Image-HD.png" alt="imac image" />
                      </a>
       ...
       {showError && (
        <div className="fixed inset-0 flex items-center justify-center bg-gray-800 bg-opacity-75">
          <div className="bg-white p-8 rounded-lg shadow-md">
            ...
            <p className="mb-4">{message && <p>{message}</p>}</p>
            <div className="flex justify-center space-x-4">
              <button onClick={() => setShowError(false)} className="bg-blue-500 text-white px-4 py-2 rounded-lg hover:bg-blue-600 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-opacity-50">Ok</button>
            </div>
          </div>
        </div>
      )}
